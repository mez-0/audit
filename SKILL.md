...
20. **file-module-organisation** — Works at the file: does its name tell the truth, does it hold one thing, is it named the way the rest of the tree is named. Dumping grounds, divergent change, temporal decomposition, naming-convention drift. See [CHECKS.md](CHECKS.md) §20. **Naming rules are normative where the language has one (PEP 8, Effective Go, RFC 430) — cohesion rules are not, and are `external` proposals.** Never report file length: no standard in the catalogue sets one.
